...
export default {
    ...
    methods: {
        async handleSubmit() {
            try {
                const setUserWP = await apiWP.connect_user(this.formUser.email, this.formUser.password)
                if (this.onSuccess) {
                    this.onSuccess(setUserWP.data.token)
                }
                localStorage.setItem("mps_moto", setUserWP.data.token)
