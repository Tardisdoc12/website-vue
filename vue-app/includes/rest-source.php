...
function myplugin_get_sources(WP_REST_Request $request) {
    ...
    $sql = "
        SELECT s.id AS subcat_id, s.title AS subcat_title, s.id_categorie AS categorie_id,
               src.id AS source_id, src.path_file, src.url_file, src.tag, src.id_wp
...
function myplugin_add_sources(WP_REST_Request $request) {
    ...
    $id_wp = intval($params['id_wp'] ?? 0);

    $inserted = $wpdb->insert(
        $table_source,
        ['id_subcategorie' => $id_subcategorie, 'path_file' => $path_file,'url_file' => $url_file, 'tag' => $tag, 'id_wp' => $id_wp],
        ['%d', '%s', '%s', '%s', '%d']
    );
